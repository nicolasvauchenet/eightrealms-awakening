...
class ReloadComponent
{
    ...
    public function reloadItem(#[LiveArg] int $characterItemId): void
    {
        ...
        $this->description .= "<p class='text-success'>{$this->screen->getCharacter()->getName()} a rechargé votre <strong>{$characterItem->getItem()->getName()}</strong> pour {$price} couronne" . ($price > 1 ? 's' : '') . ".</p>";
    }
